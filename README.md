...
三個帳號都申請完、`.env` 也填好之後，執行設定檢查指令，一次確認 CWA、TDX 與 Google 三者的狀態，不用先跑一次查詢再看它是不是報錯：

```bash
python server.py --check-config
```

CWA 與 TDX 會各發一次輕量的已驗證請求，確認金鑰真的能用；Google 只檢查
`google_credentials.json` / `google_token.json` 是否存在，**不會**觸發瀏覽器授權（那會卡住指令）。
這個指令絕對不會印出金鑰、密碼或權杖內容，只會回報「有沒有設定」與「來源在哪裡」。
權杖檔案還沒產生（顯示「🟡 待授權」）是正常狀態——第一次呼叫行事曆工具時才會引導授權。
